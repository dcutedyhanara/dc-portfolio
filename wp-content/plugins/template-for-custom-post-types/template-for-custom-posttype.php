<?php
/*
Plugin Name: Template for Custom Post Types
Plugin URI: http://www.squarelinksolutions.com/
Description: Assign page templates to any custom post types same as page.
Version: 1.0.3
Author: Rahul Gandhi
Author URI: http://www.squarelinksolutions.com/
...
define( 'WPTCP_PLUGIN_NAME', 'Template for Custom PostTypes' );
define( 'WPTCP_VERSION', '1.0.2' );
define('WPTCP_DIR', plugin_dir_path(__FILE__));
...
class WP_CustomTemplates {
	...
	function wptcp_admin_menu() {
        add_options_page(__('Posttype Template', WPTCP_PLUGIN_NAME), __('Post Type Template', WPTCP_PLUGIN_NAME), 'manage_options', 'wptcp-settings', array($this, 'wptcp_admin_page'));
    }

    function wptcp_admin_page() {
        require WPTCP_DIR . 'settings.php';
    }
    ...
    function wptcp_admin_init() {
		...
		if( isset($tcp_options['objects']) ) {
			...
			foreach ( $post_types as $post_type )
				add_meta_box( 'tcp_choose_post_template', __( 'Select Template', WPTCP_PLUGIN_NAME ), array( $this, 'wptcp_post_template'), $post_type, 'side', 'high' );
		}
    }
    ...
    function wptcp_post_template( $post )
	{
		...
		if ( $templates ) { ?>
			<label class="hidden" for="page_template"><?php _e( 'Choose Template', WPTCP_PLUGIN_NAME ); ?></label>
